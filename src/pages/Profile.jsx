                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="glass-card p-8 text-center relative overflow-hidden group">
                        ...
                        <div className="relative pt-4">
                            {/* Fix 7: avatar with alt text and skeleton loader */}
                            <div className="w-24 h-24 rounded-[28px] bg-slate-100 shadow-2xl mx-auto mb-6 flex items-center justify-center overflow-hidden border-4 border-white relative">
                                {!avatarLoaded && (
                                    <div className="absolute inset-0 bg-slate-100 animate-pulse rounded-[24px]" />
                                )}
                                <img
                                    src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(devotee.name)}&backgroundColor=f97316&textColor=fff`}
                                    className={`w-full h-full object-cover transition-opacity duration-300 ${avatarLoaded ? 'opacity-100' : 'opacity-0'}`}
                                    alt={`Avatar for ${devotee.name}`}
                ...
                <tbody className="divide-y divide-white/20">
                    {events.map((event, idx) => (
                        <tr key={event.id || idx} className="hover:bg-white/40 transition-colors group text-sm">
                            ...
                            {/* Fix 7: remark with full-text tooltip */}
                            ...
                            <td className="px-8 py-4 text-right">
                                <div className="flex items-center justify-end gap-1">
                                    <button
                                        ...
                                        className="p-2 bg-slate-50 text-slate-400 hover:bg-orange-500 hover:text-white rounded-lg transition-all"
                                        title="Preview & print receipt"
                                    >
                                        <Printer size={15} />
                                    </button>
                                    {/* Fix 7: edit event button */}
                                    <button
                                    ...
                                    {/* Fix 7: delete event (master only) */}
                                    {userRole === 'master' && (
                                        <button
